...
class SocialTest < Minitest::Test
  ...
  def test_send
    ...
    resp = @client.social.send(to: phone_number, txt: "Hello Mesage Private", from: "RubyTest", tag: "SmsTest", hidden: "Private")
  ...
  def test_status
    @client = Utility::CLIENT
    sleep(3)
    resp = @client.social.status(id: @@resp["request_id"], extended: true)
